...
st.image("pks_v2/logo-38023-scaled.jpg",width=280)
# 2 input boxes
...
with st.form("input_form"):
    ...
    nbox = st.number_input("N of Box",min_value=1,step=1,format="%d")
    ...
    if delivery_note:
        if re.fullmatch(pattern, reference):
            ...
            with engine.begin() as conn_2:
                ...
                combined.paste(dm_img, (335, qty_img.height+80))
                ...
                # dmode
                # -------------------------
                download_zip_buffer_dmode = None
                
                if dmode:
                
                    if nbox <= 0:
                        st.error("N of Box must be greater than 0")
                        st.stop()
                    ...
                    with zipfile.ZipFile(download_zip_buffer_dmode,mode="w",compression=zipfile.ZIP_DEFLATED) as zf:
                        ...
                        for i in range(1, nbox + 1):
                            ...
                            zf.writestr(
                                f"{OP_lot}_{reference}_BOX_{i}.png",
                                img_buffer.read()
                            )
                
                    download_zip_buffer_dmode.seek(0)
                
                sup_sn_check is True:
                    ...
                    st.download_button(
                        label="📦 Download D-Mode ZIP",
                        data=download_zip_buffer_dmode.getvalue(),
